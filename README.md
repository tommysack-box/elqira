...
- **Scenario Health View**: analyzes all executed requests in a scenario together and produces a structured health report that includes:
  - an API consistency checks to detect schema mismatches, naming inconsistencies, or structural drift across responses
  - error patterns analysis to surface recurring failures such as `4xx`, `5xx`, timeouts, or network-level issues
  - implicit dependencies detection to identify hidden data flows between requests, such as IDs or tokens generated by one call and required by another
